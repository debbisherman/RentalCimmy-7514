...
const { FiDownload, FiPlus, FiDollarSign, FiTag, FiEdit2, FiHome, FiCalendar } = FiIcons;

const CATEGORIES = ['Rent', 'Security Deposit', 'Utilities', 'Maintenance Fee', 'Late Fee', 'Parking', 'Other'];

const PaymentsManager = () => {
  const { payments, renters, properties, addPayment, updatePayment, role } = useApp();
  ...
  const handleSubmit = async (e) => {
    ...
    if (!error) {
      ...
      setFormData({ renter_id: '', property_id: '', amount: '', date: new Date().toISOString().split('T')[0], received_date: new Date().toISOString().split('T')[0], category: 'Rent', note: '' });
    }
    setLoading(false);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => setIsModalOpen(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2.5 rounded-xl font-semibold flex items-center gap-2 transition-all">
              ...
              return (
                <tr key={p.id} className="hover:bg-gray-50/50">
                  ...
                  <td className="px-6 py-4 text-right flex justify-end gap-2">
                    ...
                    <button onClick={() => handleDownload(p)} className="p-2 text-emerald-600 hover:bg-emerald-50 rounded-lg transition-colors">
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl">
            <div className="p-6 bg-blue-600 text-white flex justify-between items-center">
              ...
              <button onClick={() => { setIsModalOpen(false); setEditingPayment(null); }} className="text-white/80 hover:text-white"><SafeIcon icon={FiPlus} className="rotate-45 text-2xl" /></button>
            </div>
            <form onSubmit={handleSubmit} className="p-8 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Renter</label>
                  <select required className="w-full bg-gray-50 border-0 rounded-xl px-4 py-3 font-medium" value={formData.renter_id} onChange={e => setFormData({...formData, renter_id: e.target.value})}>
                ...
                <div>
                  ...
                  <select required className="w-full bg-gray-50 border-0 rounded-xl px-4 py-3 font-medium" value={formData.property_id} onChange={e => setFormData({...formData, property_id: e.target.value})}>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input required type="date" className="w-full bg-gray-50 border-0 rounded-xl px-4 py-3 font-medium" value={formData.date} onChange={e => setFormData({...formData, date: e.target.value})} />
                </div>
                <div>
                  <label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Date Received</label>
                  <input required type="date" className="w-full bg-gray-50 border-0 rounded-xl px-4 py-3 font-medium" value={formData.received_date} onChange={e => setFormData({...formData, received_date: e.target.value})} />
                </div>
              </div>
              <div>
                <label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Amount ($)</label>
                <input required type="number" step="0.01" className="w-full bg-gray-50 border-0 rounded-xl px-4 py-3 font-black text-lg" value={formData.amount} onChange={e => setFormData({...formData, amount: e.target.value})} />
              </div>
              <div>
                <label className="block text-[10px] font-black text-gray-400 uppercase mb-1">Category</label>
                <select className="w-full bg-gray-50 border-0 rounded-xl px-4 py-3 font-medium" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value})}>
                  {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
                </select>
              </div>
              <button disabled={loading} type="submit" className="w-full py-4 bg-blue-600 text-white rounded-2xl font-black shadow-lg shadow-blue-100 mt-4 uppercase tracking-widest text-sm">
